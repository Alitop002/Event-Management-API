...
from api.utils import CustmResponse, send_code_to_email
from api.serializers import EmailSerialzier, CodeSerializer, FullSignUpSerializer, LoginSerializer
...
class SendEmailRegistrationAPiView(APIView):
    ...
    def post(self, request):
        ...
        return CustmResponse.success(
            status=True,
            message="Confirmations code has sent to you email.",
            data=user.token()
            )
    

@extend_schema(tags=['Auth'])
class CodeVerifiredAPiView(APIView):
